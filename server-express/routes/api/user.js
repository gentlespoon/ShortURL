...
router.post('/login', (req, res, next) => {
  ...
  db_user.query("SELECT account_id, password FROM accounts WHERE (login_name=? OR email=?) AND banned_time<?", [req.body.email, req.body.email, moment().toISOString()], (err, result, fields) => {
...
router.post('/registerCheck', (req, res, next) => {
  // validate input type
  if (req.body.email) {
...
router.post('/register', (req, res, next) => {
  ...
  db_user.query("SELECT account_id, login_name FROM accounts WHERE email=? OR account_id=?", [req.body.email, account_id], (err, result, fields) => {
    ...
    db_user.query("INSERT INTO accounts (account_id, email, password, referrer, register_time) VALUES (?, ?, ?, ? ,?)", [account_id, req.body.email, hash, req.session.account_id, moment().toISOString()], (err, result, fields) => {
      if (err) return res.send(api(0, err));
      // schedule a welcome email
      let mail = {
        from: 'me@gentlespoon.com',
        to: req.body.email,
        subject: 'Welcome to GtSpn.com ShortURL',
    ...
    if (result.length) {
      ...
      db_user.query('INSERT INTO tokens (purpose, account_id, token, expire) VALUES (?, ?, ?, ?)', ['reset', account_id, token, moment().add(1, 'd').toISOString()], (err, result, fields) => {
        ...
        let mail = {
          from: 'me@gentlespoon.com',
          to: req.body.email,
          subject: 'Reset your password - GtSpn.com ShortURL',
          html: email.template('forget').split('{{token}}').join(token)
        };
        email.send(mail);
        return res.send(api(1, 'If your email address is in our database, we will send you a link to reset your password.'));
      })
    } else {
      return res.send(api(1, 'If your email address is in our database, we will send you a link to reset your password.'));
    }
