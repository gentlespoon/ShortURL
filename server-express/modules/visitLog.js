...
class VisitLog {
  ...
  log(req) {
    ...
    db.query('INSERT INTO history (request_url, short_url, long_url, request_time, ip, location, user_agent) VALUES (?, ?, ?, ?, ?, ?, ?)', [req.url, req.lookup_url, req.target_url, moment().toISOString(), req.ip, req.location, req.headers["user-agent"]], (err, result, fields) => {
      if (err) throw err;
    });
  }
}
